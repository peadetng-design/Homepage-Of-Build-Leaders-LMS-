...
const SettingsView: React.FC<{ user: User, currentTheme: string, onThemeChange: (t: string) => void, onChangePassword: () => void }> = ({ user, currentTheme, onThemeChange, onChangePassword }) => {
  ...
  const languages = [
    { code: 'en-US', name: 'English (US)' }, { code: 'en-GB', name: 'English (UK)' }, { code: 'es', name: 'Español' }, { code: 'fr', name: 'Français' }, { code: 'de', name: 'Deutsch' }, { code: 'zh', name: '中文 (简体)' }, { code: 'ja', name: '日本語' }, { code: 'ko', name: '한국어' }, { code: 'ar', name: 'العربية' }, { code: 'pt', name: 'Português' }, { code: 'ru', name: 'Русский' }, { code: 'hi', name: 'हिन्दी' }, { code: 'bn', name: 'বাংলা' }, { code: 'id', name: 'Bahasa Indonesia' }, { code: 'tr', name: 'Türkçe' }, { code: 'it', name: 'Italiano' }, { code: 'nl', name: 'Nederlands' }, { code: 'pl', name: 'Polski' }, { code: 'vi', name: 'Tiếng Việt' }, { code: 'th', name: 'ไทย' }, { code: 'sw', name: 'Kiswahili' }, { code: 'he', name: 'עברית' },
  ];

  const timezones = [
    "Pacific/Honolulu", "America/Anchorage", "America/Los_Angeles", "America/Phoenix", "America/Denver", "America/Chicago", "America/New_York", "America/Sao_Paulo", "America/St_Johns", "UTC", "Europe/London", "Europe/Paris", "Europe/Berlin", "Europe/Moscow", "Africa/Cairo", "Africa/Johannesburg", "Asia/Dubai", "Asia/Karachi", "Asia/Kolkata", "Asia/Dhaka", "Asia/Bangkok", "Asia/Singapore", "Asia/Hong_Kong", "Asia/Tokyo", "Asia/Seoul", "Australia/Perth", "Australia/Adelaide", "Australia/Sydney", "Pacific/Auckland", "Pacific/Fiji"
  ];
      ...
      <div className="bg-white p-4 md:p-12 rounded-b-[1.5rem] md:rounded-b-[3rem] shadow-2xl border-x border-b border-gray-100 space-y-6 md:space-y-12">
        ...
        <section className="space-y-3 md:space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-4">
            <ControlRow icon={Languages} title="System Language" desc="Dialect for UI interaction.">
              <select value={lang} onChange={(e) => setLang(e.target.value)} className="bg-gray-50 border-2 border-gray-100 rounded-lg px-2 py-1 md:px-4 md:py-2 font-bold text-gray-800 outline-none focus:border-royal-500 transition-all w-full md:w-auto text-[10px] md:text-sm">
                {languages.map(l => <option key={l.code} value={l.code}>{l.name}</option>)}
              </select>
            </ControlRow>
            <ControlRow icon={Clock} title="Time Zone" desc="Identified automatically.">
              <select value={timezone} onChange={(e) => setTimezone(e.target.value)} className="bg-gray-50 border-2 border-gray-100 rounded-lg px-2 py-1 md:px-4 md:py-2 font-bold text-gray-800 outline-none focus:border-royal-500 transition-all w-full md:max-w-[200px] text-[10px] md:text-sm">
        ...
        <section className="space-y-3 md:space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2 md:gap-4">
            {themes.map(t => (
              <button key={t.id} onClick={() => onThemeChange(t.id)} className={`flex flex-col items-center gap-1.5 md:gap-3 p-3 md:p-6 rounded-[1rem] md:rounded-[2rem] border-2 md:border-4 transition-all ${currentTheme === t.id ? 'border-royal-600 ring-2 md:ring-4 ring-royal-50 scale-105' : 'border-gray-100 hover:border-royal-200'}`}>
                <div className={`w-8 h-8 md:w-12 md:h-12 rounded-lg md:rounded-2xl flex items-center justify-center shadow-lg ${t.bg} ${t.text}`}><t.icon size={16} className="md:size-6" /></div>
        ...
        <section className="space-y-3 md:space-y-6">
          ...
          <div className="space-y-2 md:space-y-4">
            <ControlRow icon={Save} title="Intelligence AutoSave" desc="Preserve progress." colorClass="text-emerald-600"><Toggle enabled={autoSave} setEnabled={setAutoSave} /></ControlRow>
            <ControlRow icon={RefreshCcw} title="Session Recovery" desc="Restore unsubmitted." colorClass="text-orange-500"><Toggle enabled={recovery} setEnabled={setRecovery} /></ControlRow>
            <ControlRow icon={Zap} title="Fast Load Engine" desc="Instant transitions." colorClass="text-gold-600"><Toggle enabled={fastLoad} setEnabled={setFastLoad} /></ControlRow>
            <ControlRow icon={Monitor} title="Device Sync" desc="Cross-device status." colorClass="text-indigo-600"><Toggle enabled={sync} setEnabled={setSync} /></ControlRow>
          </div>
        </section>
        ...
        <section className="pt-2 md:pt-6">
          <div className="bg-royal-50 p-4 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border-2 border-royal-100 flex flex-col md:flex-row items-center justify-between gap-4 md:gap-6">
            <div className="flex items-center gap-3 md:gap-4">
              <div className="p-2 md:p-4 bg-royal-800 text-white rounded-xl md:rounded-2xl shadow-xl shrink-0"><Key size={20} className="md:size-6" /></div>
              <div><h4 className="font-black text-gray-900 uppercase text-[11px] md:text-base">System Access</h4><p className="text-[9px] md:text-sm text-royal-600 font-medium">Update master password.</p></div>
            </div>
            <button onClick={onChangePassword} className="w-full md:w-auto px-6 md:px-10 py-3 md:py-4 bg-royal-800 text-white font-black rounded-xl md:rounded-2xl hover:bg-black transition-all shadow-xl border-b-4 border-royal-950 flex items-center justify-center gap-2 uppercase text-[9px] md:text-xs">Update Security Code</button>
          </div>
        </section>
      </div>
    </div>
        ...
        <div className="absolute -bottom-16 left-1/2 -translate-x-1/2 md:left-12 md:translate-x-0">
          <div className="w-32 h-32 md:w-40 md:h-40 rounded-[2.5rem] bg-white p-2 shadow-2xl">
            <div className="w-full h-full rounded-[2rem] bg-royal-100 flex items-center justify-center overflow-hidden border-4 border-royal-50">
              {user.avatarUrl ? <img src={user.avatarUrl} alt={user.name} className="w-full h-full object-cover" /> : <UserCircle size={80} className="text-royal-300" />}
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-12">
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <span className="px-4 py-1 bg-gold-50 text-gold-600 rounded-full text-xs font-black uppercase tracking-widest flex items-center gap-2 border border-gold-100"><Shield size={12} /> VERIFIED ACCOUNT</span>
            </div>
          </div>
          <button onClick={onEdit} className="w-full md:w-auto px-8 py-3 bg-royal-800 text-white font-black rounded-2xl hover:bg-black transition-all flex items-center justify-center gap-3 shadow-xl border-b-4 border-royal-950 uppercase text-sm"><Edit3 size={18} /> Edit Profile</button>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4 group">
                ...
                <div><p className="text-[10px] font-black text-gray-400 uppercase">Email Address</p><p className="font-bold text-gray-800 text-sm md:text-base">{user.email}</p></div>
              </div>
              <div className="flex items-center gap-4 group">
                ...
                <div><p className="text-[10px] font-black text-gray-400 uppercase">Primary Role</p><p className="font-bold text-gray-800 uppercase tracking-wide text-sm md:text-base">{user.role.replace('_', ' ')}</p></div>
              </div>
            </div>
          </div>
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4 group">
                ...
                <div><p className="text-[10px] font-black text-gray-400 uppercase">Last Recorded Session</p><p className="font-bold text-gray-800 text-sm md:text-base">{user.lastLogin ? new Date(user.lastLogin).toLocaleString() : 'First Session'}</p></div>
              </div>
              <div className="flex items-center gap-4 group">
                ...
                <div><p className="text-[10px] font-black text-gray-400 uppercase">Certificates Earned</p><p className="font-bold text-gray-800 text-sm md:text-base">{completedCount} Validated Credentials</p></div>
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="max-w-2xl mx-auto animate-in zoom-in-95 duration-500">
      <div className="bg-white p-8 md:p-12 rounded-[2.5rem] shadow-2xl border border-gray-100">
        <div className="flex justify-between items-center mb-10 border-b-2 border-gray-50 pb-6"><h2 className="text-2xl md:text-3xl font-serif font-black text-gray-900 uppercase tracking-tight">Edit Identity</h2><button onClick={onCancel} className="p-2 text-gray-400 hover:text-gray-900 transition-colors"><X size={24} /></button></div>
        <form onSubmit={handleSave} className="space-y-8">
          <div className="flex flex-col items-center gap-4">
            <div className="relative group/avatar">
              <div className="w-32 h-32 rounded-[2rem] bg-royal-50 flex items-center justify-center overflow-hidden border-4 border-white shadow-xl">{previewAvatar ? <img src={previewAvatar} alt="preview" className="w-full h-full object-cover" /> : <UserCircle size={64} className="text-royal-200" />}</div>
              <button type="button" onClick={() => document.getElementById('avatar-input')?.click()} className="absolute inset-0 bg-black/40 opacity-0 group-hover/avatar:opacity-100 transition-opacity flex items-center justify-center rounded-[2rem]"><Camera size={32} className="text-white" /></button>
          ...
          <div className="space-y-6">
            <div><label className="text-xs font-black text-gray-500 uppercase tracking-widest ml-1 block mb-2">Full Name</label><input required className="w-full p-4 bg-gray-50 border-2 border-transparent rounded-2xl focus:bg-white focus:border-royal-500 outline-none transition-all font-bold text-gray-800" value={name} onChange={e => setName(e.target.value)} /></div>
            <div><label className="text-xs font-black text-gray-500 uppercase tracking-widest ml-1 block mb-2">Email Address</label><input required type="email" className="w-full p-4 bg-gray-50 border-2 border-transparent rounded-2xl focus:bg-white focus:border-royal-500 outline-none transition-all font-bold text-gray-800" value={email} onChange={e => setEmail(e.target.value)} /></div>
          </div>
          <div className="pt-6 flex gap-4"><button type="button" onClick={onCancel} className="flex-1 py-4 bg-gray-100 text-gray-600 font-black rounded-2xl hover:bg-gray-200 transition-all uppercase text-xs">Cancel</button><button type="submit" disabled={isSaving} className="flex-[2] py-4 bg-royal-800 text-white font-black rounded-2xl hover:bg-black transition-all shadow-xl flex items-center justify-center gap-2 uppercase text-xs border-b-4 border-royal-950">{isSaving ? <Loader2 className="animate-spin" size={20} /> : <Save size={20} />} Update Identity</button></div>
        </form>
      </div>
    </div>
  );
...
const ScrollReveal = ({ children, className = "", delay = 0 }: { children?: React.ReactNode, className?: string, delay?: number }) => {
  ...
  return (
    <div ref={ref} style={{ transitionDelay: `${delay}ms` }} className={`${className} transition-all duration-1000 ease-out transform ${isVisible ? 'opacity-100 translate-y-0 scale-100 filter brightness-110 contrast-125 saturate-125' : 'opacity-100 translate-y-4 scale-95 filter brightness-100 contrast-100'}`}>{children}</div>
  );
};

const MiniPieChart = ({ percent, color, bgColor }: { percent: number, color: string, bgColor: string }) => {
  const radius = 12; const circumference = 2 * Math.PI * radius; const strokeDashoffset = circumference - (percent / 100) * circumference;
  return (
    <div className="relative w-10 h-10 flex items-center justify-center shrink-0"><svg className="w-full h-full transform -rotate-90"><circle cx="20" cy="20" r={radius} stroke={bgColor} strokeWidth="3" fill="none" /><circle cx="20" cy="20" r={radius} stroke={color} strokeWidth="3" fill="none" strokeDasharray={circumference} strokeDashoffset={strokeDashoffset} strokeLinecap="round" className="transition-all duration-1000" /></svg><span className="absolute text-[8px] font-black" style={{ color }}>{Math.round(percent)}%</span></div>
  );
};
  ...
  return (
    <div className="relative inline-block">
      <button onClick={() => setIsOpen(!isOpen)} className={`flex items-center gap-3 px-8 py-3 rounded-2xl font-black text-xs md:text-sm tracking-[0.3em] transition-all transform hover:-translate-y-1 active:scale-95 shadow-2xl border-b-[8px] ${primaryColorClass} text-white uppercase group overflow-hidden relative z-[101]`}>
        ...
        <span className="relative z-10">{label}</span><ChevronDown size={18} className={`relative z-10 transition-transform duration-500 ${isOpen ? 'rotate-180' : ''}`} />
      </button>
      ...
      {isOpen && (
        <div className={`absolute left-1/2 -translate-x-1/2 md:left-0 md:translate-x-0 w-[16rem] md:w-[20rem] bg-white rounded-[2.2rem] shadow-[0_40px_100px_-15px_rgba(0,0,0,0.4)] border border-gray-100 p-2.5 md:p-3 z-[150] animate-in fade-in zoom-in-95 duration-300 ${direction === 'up' ? 'bottom-full mb-4 origin-bottom slide-in-from-bottom-4' : 'top-full mt-4 origin-top slide-in-from-top-4'}`}>
          <div className="flex flex-col gap-1 md:gap-1.5">
            {items.map((item, idx) => (
              <button key={idx} onClick={() => { item.action(); setIsOpen(false); }} className={`flex items-center gap-3 md:gap-4 p-3 md:p-4 rounded-[1.4rem] md:rounded-[1.6rem] transition-all duration-300 group/item hover:scale-[1.03] ${item.hoverBg} text-left active:scale-95`}><div className={`p-2 md:p-2.5 rounded-xl shadow-lg transition-all duration-500 ${item.color} group-hover/item:scale-110 group-hover/item:shadow-[0_0_20px_rgba(0,0,0,0.2)]`} style={{ filter: `drop-shadow(0 0 8px ${item.glowColor}90)` }}><item.icon size={16} /></div><div className="flex flex-col"><span className="font-black text-[10px] md:text-sm uppercase tracking-[0.1em] text-gray-700 group-hover/item:text-gray-950 transition-colors leading-tight">{item.label}</span><div className="h-0.5 w-0 group-hover/item:w-full transition-all duration-500 rounded-full mt-0.5" style={{ backgroundColor: item.glowColor }}></div></div></button>
            ))}
          </div>
        </div>
      )}
...
interface DropdownOption { label: string; icon: React.ElementType; action: () => void; color: string; glowColor: string; hoverBg: string; }
...
const Dashboard: React.FC<DashboardProps> = ({ user, onUpdateUser, onChangePasswordClick, initialView = 'dashboard' }) => {
  ...
  const formatDigitalTime = (seconds: number) => { const h = Math.floor(seconds / 3600); const m = Math.floor((seconds % 3600) / 60); const s = seconds % 60; return [h, m, s].map(v => v.toString().padStart(2, '0')).join(':'); };
      ...
      <div className="space-y-8 md:space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-700 pb-32">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-14 relative z-0 lg:max-w-[90%] lg:mx-auto">
              <div className="lg:col-span-2 space-y-8 md:space-y-14">
                  ...
                  <div className="p-3 md:p-4 bg-white rounded-[2rem] md:rounded-[2.5rem] border-4 border-royal-100 shadow-[0_40px_100px_-30px_rgba(0,0,0,0.1)] flex flex-col gap-3 group relative overflow-hidden"><div className="flex flex-col md:flex-row items-center gap-4 relative z-10"><div className="p-3 bg-royal-900 rounded-[1.4rem] text-gold-400 shrink-0 shadow-2xl border-b-[5px] border-royal-950"><Activity size={22}/></div><div className="text-center md:text-left flex-1"><h4 className="font-serif font-black text-gray-950 text-base md:text-xl uppercase tracking-tighter">Performance Intelligence</h4><div className="flex flex-wrap items-center justify-center md:justify-start gap-2 mt-1"><p className="text-[10px] md:text-xs text-gray-500 font-bold opacity-80">Track your path to leadership mastery with real-time analytics.</p><button onClick={() => setInternalView('performance-report')} className="px-7 py-2.5 bg-royal-800 hover:bg-black text-white font-black rounded-xl text-xs uppercase tracking-[0.2em] transition-all transform hover:-translate-y-1 border-b-2 border-royal-950 flex items-center gap-1.5">FULL REPORT <ArrowRight size={14} /></button></div></div></div><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 pt-3 border-t border-gray-100"><ScrollReveal className="lg:col-span-2" delay={100}><div className="space-y-2 bg-white p-2.5 rounded-2xl border-4 border-indigo-400 shadow-sm group/stat hover:border-indigo-600 transition-all"><div className="flex justify-between items-start"><div className="flex items-center gap-2.5"><div className="p-2 bg-royal-100 text-royal-700 rounded-lg group-hover/stat:bg-royal-600 group-hover/stat:text-white transition-colors shadow-sm"><Layers size={16} /></div><div><h5 className="text-[8px] font-black text-royal-600 uppercase tracking-widest leading-none">NUMBER OF MODULES</h5><p className="text-[9px] font-bold text-gray-400 truncate max-w-[80px]">{stats.modulesCompleted} Won</p></div></div><span className="text-xl font-black text-royal-950 leading-none">{completionPercent}%</span></div><div className="h-2.5 w-full bg-gray-50 rounded-full border border-royal-50 overflow-hidden p-0.5"><div className="h-full bg-gradient-to-r from-royal-600 to-royal-400 rounded-full transition-all duration-1000" style={{ width: `${completionPercent}%` }} /></div></div></ScrollReveal><ScrollReveal delay={200}><div className="bg-white p-2.5 rounded-2xl border-4 border-gold-400 flex items-center gap-2.5 shadow-sm group/stat hover:border-gold-600 transition-all h-full"><div className="p-2 bg-gold-50 text-gold-600 rounded-lg group-hover/stat:bg-gold-500 group-hover/stat:text-white transition-colors shadow-sm shrink-0"><Clock size={16} /></div><div className="min-w-0"><h5 className="text-[8px] font-black text-royal-600 uppercase tracking-widest leading-none mb-1">TIME DURATION</h5><span className="text-xl font-black text-royal-950 leading-none">{formatDigitalTime(stats.totalTime)}</span></div></div></ScrollReveal><ScrollReveal delay={300}><div className="bg-white p-2.5 rounded-2xl border-4 border-royal-500 flex items-center gap-2.5 shadow-sm group/stat hover:border-royal-700 transition-all h-full"><div className="min-w-0 flex items-center gap-1.5"><MiniPieChart percent={stats.avgScore} color="#4f46e5" bgColor="#e0e7ff" /><div><h5 className="text-[8px] font-black text-royal-600 uppercase tracking-widest leading-none mb-1">AVG SCORE (%)</h5><span className="text-xl font-black text-royal-950 leading-none">{stats.avgScore}%</span></div></div></div></ScrollReveal></div><div className="grid grid-cols-1 md:grid-cols-2 gap-3"><ScrollReveal delay={400} className="w-full"><div className="flex items-center gap-2.5 p-2.5 bg-white border-4 border-amber-400 rounded-2xl shadow-sm hover:shadow-xl hover:border-amber-500 transition-all group/time"><div className="p-2 bg-amber-50 text-amber-600 rounded-lg group-hover/time:bg-amber-500 group-hover/time:text-white transition-colors shadow-sm"><Award size={20} /></div><div><h5 className="text-[8px] font-black text-royal-600 uppercase tracking-widest leading-none mb-1">LAST LESSON SCORE</h5><p className="text-lg md:text-xl font-black text-gray-900 leading-none">{stats.lastLessonScore}</p></div></div></ScrollReveal><ScrollReveal delay={500} className="w-full"><div className="flex items-center gap-2.5 p-2.5 bg-white border-4 border-emerald-400 rounded-2xl shadow-sm hover:shadow-xl hover:border-emerald-500 transition-all group/time"><div className="p-2 bg-emerald-50 text-emerald-600 rounded-lg group-hover/time:bg-emerald-500 group-hover/time:text-white transition-colors shadow-sm"><History size={20} /></div><div><h5 className="text-[8px] font-black text-royal-600 uppercase tracking-widest leading-none mb-1">LAST LESSON TIME</h5><p className="text-lg md:text-xl font-black text-gray-900 leading-none">{formatDigitalTime(stats.lastLessonTime)}</p></div></div></ScrollReveal></div></div>
              </div>
              <div className="space-y-8 md:space-y-14"><div className="bg-white rounded-[2.5rem] p-6 md:p-8 border-2 border-gray-50 shadow-2xl"><h3 className="font-serif font-black text-gray-900 text-base md:text-2xl uppercase tracking-[0.3em] mb-8 flex items-center gap-3"><div className="p-2 bg-royal-900 text-white rounded-2xl shadow-lg"><MessageSquare size={22} /></div> RECENT CHATS</h3><div className="space-y-6">{recentChats.map(m => (<div key={m.id} className="p-4 rounded-3xl bg-gray-50/50 hover:bg-white border-2 border-transparent hover:border-royal-100 hover:shadow-xl transition-all group cursor-pointer" onClick={() => setInternalView('chat')}><div className="flex gap-4 items-start"><div className="w-10 h-10 md:w-12 md:h-12 rounded-2xl bg-white flex items-center justify-center font-black text-royal-700 border border-royal-100 shrink-0 shadow-sm relative group-hover:scale-110 transition-transform">{m.senderName.charAt(0)}<div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-green-500 rounded-full border-2 border-white shadow-sm"></div></div><div className="min-w-0 flex-1"><div className="flex justify-between items-center mb-1.5"><span className="text-[10px] font-black text-royal-600 uppercase tracking-[0.2em]">{m.senderName}</span><span className="text-[9px] font-bold text-gray-400"><Clock size={10} /> {new Date(m.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span></div><p className="text-sm md:text-base font-serif font-black text-royal-950 leading-snug line-clamp-2 tracking-tight">"{m.text}"</p></div></div></div>))}</div><button onClick={() => setInternalView('chat')} className="w-full mt-10 py-5 bg-royal-900 text-white text-[10px] font-black rounded-[1.5rem] hover:bg-black uppercase tracking-[0.3em] transition-all shadow-2xl border-b-[5px] border-black">OPEN CHATS APP</button></div></div>
          </div>
