...
import { Upload, X, Loader2, Save, Plus, Trash2, Edit3, BookOpen, File as FileIcon, CheckCircle, HelpCircle, ArrowRight, AlertTriangle, Layers, BadgeCheck, PenTool, Check, Info, Library, Layout, ListChecks, Download, AlertCircle, ArrowLeft, Sparkles, FileText } from 'lucide-react';
...
const LessonUpload: React.FC<LessonUploadProps> = ({ currentUser, onSuccess, onCancel, initialData, initialContentType = 'lesson' }) => {
  ...
  // Manual Creation State (Hierarchical)
  ...
  const validateManual = () => {
      if (activeManualStep === 1) {
          ...
          if ((manualCourse.about?.length || 0) < 1) return "Course requires at least 1 segment.";
      }
      if (activeManualStep === 2) {
          if (!manualModule.title) return "Module Title is mandatory.";
          if ((manualModule.about?.length || 0) < 1) return "Module requires at least 1 segment.";
      }
      if (activeManualStep === 3) {
          if (!manualLesson.title) return "Lesson Title is mandatory.";
          if ((manualLesson.about?.length || 0) < 1) return "Lesson requires at least 1 segment.";
      }
      return null;
  };
  ...
  const saveManual = async () => {
    ...
    if (activeManualStep < 3) { setActiveManualStep(prev => prev + 1 as any); return; }
    try {
        const cId = manualCourse.id || crypto.randomUUID();
        const mId = manualModule.id || crypto.randomUUID();
        const lId = manualLesson.id || crypto.randomUUID();
        await lessonService.publishCourse({ ...manualCourse, id: cId } as Course);
        await lessonService.publishModule({ ...manualModule, id: mId, courseId: cId, lessonIds: [lId] } as Module);
        await lessonService.publishLesson({ ...manualLesson, id: lId, moduleId: mId, author: currentUser.name, authorId: currentUser.id, created_at: new Date().toISOString(), updated_at: new Date().toISOString(), status: 'published', views: 0 } as Lesson);
        onSuccess();
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm animate-in fade-in">
        <div className="bg-white rounded-[3rem] shadow-[0_50px_120px_-30px_rgba(0,0,0,0.5)] w-full max-w-6xl h-[90vh] overflow-hidden flex flex-col border-4 border-white/20">
            
            <div className="bg-royal-900 p-8 text-white relative shrink-0">
                <div className="absolute inset-0 opacity-10 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')]"></div>
                <div className="relative z-10 flex justify-between items-center">
                    <div className="flex items-center gap-6">
                        <div className="p-4 bg-white/10 rounded-3xl backdrop-blur-md shadow-2xl border border-white/10 text-gold-500"><Upload size={40} /></div>
                        <div><h2 className="text-3xl font-serif font-black uppercase tracking-tight">Structured Curriculum Engine</h2><p className="text-royal-200 text-xs font-black uppercase tracking-[0.4em] mt-1">Excel & Manual Build Protocol</p></div>
                    </div>
                    <div className="flex gap-2">
                        <div className="bg-royal-950 p-1 rounded-2xl flex border border-white/10">
                            <button onClick={() => { setMetricMode('bulk'); setDraft(null); }} className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase transition-all ${metricMode === 'bulk' ? 'bg-white text-royal-900 shadow-xl' : 'text-royal-300 hover:text-white'}`}>Excel Import</button>
                            <button onClick={() => { setMetricMode('manual'); setDraft(null); }} className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase transition-all ${metricMode === 'manual' ? 'bg-white text-royal-900 shadow-xl' : 'text-royal-300 hover:text-white'}`}>Manual Builder</button>
                        </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-12 bg-[#fdfdfd]">
                {error && (
                    <div className="bg-red-50 border-4 border-red-100 p-8 rounded-[2.5rem] mb-12 flex items-start gap-6 shadow-xl animate-in slide-in-from-top-4">
                        ...
                        <div><h4 className="text-red-900 font-black text-xl uppercase mb-2">Protocol Violation</h4><p className="text-red-700 font-bold leading-relaxed">{error}</p></div>
                    </div>
                )}
                ...
                {metricMode === 'bulk' ? (
                        ...
                        {!draft ? (
                            <div className="space-y-8 animate-in fade-in">
                                <div className="bg-indigo-50 border-4 border-indigo-100 p-10 rounded-[3rem] relative overflow-hidden group">
                                    ...
                                    <div className="relative z-10">
                                        ...
                                        <p className="text-indigo-700 font-medium leading-relaxed max-w-xl">Upload your consolidated .xlsx workbook following the 8-sheet protocol: Metadata + About segments for Course, Module, and Lesson.</p>
                                        <div className="mt-10 flex gap-4">
                                            <button className="px-8 py-4 bg-indigo-600 text-white font-black rounded-2xl shadow-xl border-b-4 border-indigo-900 hover:bg-indigo-700 transition-all flex items-center gap-3">
                                                <Download size={20} /> DOWNLOAD PROTOCOL (.XLSX)
                                            </button>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="border-8 border-dashed border-gray-100 rounded-[3.5rem] p-24 text-center hover:bg-gray-50/50 hover:border-indigo-100 transition-all cursor-pointer relative group">
                                    <input type="file" accept=".xlsx,.csv" onChange={(e) => setFile(e.target.files?.[0] || null)} className="absolute inset-0 opacity-0 cursor-pointer z-10" />
                                    <div className="flex flex-col items-center gap-6 group-hover:scale-105 transition-transform">
                                        ...
                                        <div><p className="text-2xl font-serif font-black text-gray-800 uppercase tracking-tighter">{file ? file.name : "DEPOSIT DATA WORKBOOK"}</p><p className="text-gray-400 font-bold uppercase tracking-widest text-[10px] mt-2">Maximum file size: 25MB • .xlsx/.csv</p></div>
                                    </div>
                                </div>
                                ...
                                {file && (
                                    <button onClick={processBulkFile} disabled={isParsing} className="w-full py-6 bg-royal-950 text-white font-black rounded-[2rem] shadow-2xl border-b-8 border-black hover:bg-black transition-all flex items-center justify-center gap-6 text-2xl group">
                                        {isParsing ? <Loader2 className="animate-spin" size={32}/> : <CheckCircle size={32} className="text-gold-500" />} {isParsing ? "Analyzing Protocol..." : "START SYSTEM IMPORT"}
                                    </button>
                                )}
                            </div>
                        ) : (
                            <div className="space-y-12 animate-in fade-in duration-700">
                                <div className="flex justify-between items-end border-b-4 border-gray-50 pb-8">
                                    <div><h3 className="text-4xl font-serif font-black text-gray-950 uppercase tracking-tighter">Integrity Check Completed</h3><p className="text-gray-400 font-bold uppercase tracking-widest text-xs mt-2">Preview before system commit</p></div>
                                    <button onClick={() => setDraft(null)} className="px-6 py-2 bg-red-50 text-red-600 font-black rounded-xl text-[10px] uppercase hover:bg-red-100 transition-all">DISCARD</button>
                                </div>

                                {draft.errors.length > 0 && (
                                    <div className="bg-red-50 border-4 border-red-100 rounded-[2.5rem] shadow-xl overflow-hidden">
                                        <div className="bg-red-600 p-6 text-white font-black uppercase text-sm tracking-widest flex items-center gap-3"><AlertTriangle size={24}/> Row-Level Violations Identified</div>
                                        <div className="max-h-60 overflow-y-auto">
                                            <table className="w-full text-left">
                                                <thead><tr className="bg-red-100 text-[10px] font-black uppercase text-red-600 tracking-widest border-b border-red-200"><th className="p-4">Sheet</th><th className="p-4">Row</th><th className="p-4">Detail</th></tr></thead>
                                                <tbody className="divide-y divide-red-100 text-sm font-bold">
                                                    {draft.errors.map((e,i) => (
                                                        <tr key={i}><td className="p-4"><span className="bg-white px-2 py-1 rounded text-[10px]">{e.sheet}</span></td><td className="p-4 font-mono text-red-700">#{e.row}</td><td className="p-4 text-red-800">{e.message}</td></tr>
                                                    ))}
                                                </tbody>
                                            </table>
                                        </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div className="bg-indigo-900 p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden">
                                        ...
                                        <h4 className="text-[10px] font-black text-indigo-300 uppercase tracking-[0.4em] mb-4">Course Identity</h4>
                                        ...
                                        <div className="mt-8 flex gap-4">
                                            <div className="px-4 py-2 bg-white/10 rounded-xl text-xs font-black uppercase">{draft.courseMetadata?.about.length} Segments</div>
                                            <div className="px-4 py-2 bg-white/10 rounded-xl text-xs font-black uppercase">{draft.modules.length} Modules</div>
                                        </div>
                                    </div>
                                    <div className="space-y-4">
                                        <h4 className="font-black text-gray-400 uppercase tracking-widest text-[10px]">Module Architecture</h4>
                                        {draft.modules.map((m,i) => (
                                            <div key={i} className="bg-white border-4 border-gray-50 p-6 rounded-[2.2rem] shadow-lg flex justify-between items-center">
                                                <div className="flex items-center gap-4"><div className="p-3 bg-indigo-50 text-indigo-600 rounded-2xl"><Layers size={24}/></div><div><h4 className="font-serif font-black text-gray-900 uppercase tracking-tighter leading-tight">{m.title}</h4><p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{m.about.length} Structured Segments</p></div></div>
                                ...
                                <button onClick={() => lessonService.commitDraft(draft, currentUser).then(onSuccess)} disabled={!draft.isValid} className="w-full py-6 bg-indigo-600 text-white font-black rounded-[2rem] shadow-2xl border-b-8 border-indigo-900 hover:bg-indigo-700 transition-all uppercase tracking-[0.4em] disabled:opacity-50">COMMIT VERIFIED HIERARCHY</button>
                            </div>
                        )}
                    </div>
                ) : (
                    <div className="max-w-5xl mx-auto space-y-12 animate-in fade-in">
                        ...
                        {activeManualStep === 1 && (
                            <div className="space-y-8 animate-in slide-in-from-right-4">
                                <div className="bg-white p-10 rounded-[3rem] border-4 border-gray-100 shadow-sm space-y-6">
                                    <h3 className="text-2xl font-serif font-black text-gray-900 uppercase flex items-center gap-3"><Library className="text-indigo-600"/> 1. COURSE IDENTITY</h3>
                        ...
                        {activeManualStep === 2 && (
                            <div className="space-y-8 animate-in slide-in-from-right-4">
                                <div className="bg-white p-10 rounded-[3rem] border-4 border-gray-100 shadow-sm space-y-6">
                                    <h3 className="text-2xl font-serif font-black text-gray-900 uppercase flex items-center gap-3"><Layers className="text-indigo-600"/> 2. MODULE CONTEXT</h3>
                                    <div className="grid grid-cols-2 gap-6">
                                        <div className="col-span-2"><label className={labelClass}>Module Title</label><input className={inputClass} value={manualModule.title} onChange={e => setManualModule({...manualModule, title: e.target.value})} placeholder="e.g. Genesis Mastery" /></div>
                                        ...
                                        <div><label className={labelClass}>Total Lessons Required</label><input type="number" className={inputClass} value={manualModule.totalLessonsRequired} onChange={e => setManualModule({...manualModule, totalLessonsRequired: parseInt(e.target.value)})} /></div>
                                    </div>
                                </div>
                        ...
                        {activeManualStep === 3 && (
                            <div className="space-y-8 animate-in slide-in-from-right-4 pb-20">
                                <div className="bg-white p-10 rounded-[3rem] border-4 border-gray-100 shadow-sm space-y-6">
                                    ...
                                    <div className="grid grid-cols-2 gap-6">
                                        <div className="col-span-2"><label className={labelClass}>Lesson Title</label><input className={inputClass} value={manualLesson.title} onChange={e => setManualLesson({...manualLesson, title: e.target.value})} placeholder="e.g. Lesson 1: Order from Chaos" /></div>
                                        ...
                                        <div><label className={labelClass}>Target Audience</label><select className={inputClass} value={manualLesson.targetAudience} onChange={e => setManualLesson({...manualLesson, targetAudience: e.target.value as any})}><option>All</option><option>Student</option><option>Mentor</option></select></div>
                                    </div>
                                </div>
                                <AboutSegmentBuilder segments={manualLesson.about || []} type="lesson" onAdd={() => addAboutSegment('lesson')} onUpdate={(idx, upd) => updateAboutSegment('lesson', idx, upd)} onRemove={(idx) => removeAboutSegment('lesson', idx)} min={1} />
                            </div>
                        )}
                        ...
                        <div className="fixed bottom-12 left-1/2 -translate-x-1/2 flex items-center gap-4 z-50">
                            ...
                            <button onClick={saveManual} className="px-20 py-6 bg-indigo-600 text-white font-black rounded-[2.5rem] shadow-2xl border-b-8 border-indigo-950 uppercase tracking-[0.4em] text-sm hover:bg-indigo-700 transition-all flex items-center gap-4 active:scale-95">
    ...
    return (
        <div className="bg-white p-10 rounded-[3rem] border-4 border-gray-50 shadow-sm space-y-8">
            <div className="flex justify-between items-center border-b-4 border-gray-50 pb-6">
                <div>
                    <h3 className="text-xl font-serif font-black text-gray-900 uppercase flex items-center gap-3"><Sparkles size={24} className="text-gold-500" /> "ABOUT THIS {type.toUpperCase()}" Segments ({segments.length})</h3>
                    <p className={`text-[10px] font-black uppercase tracking-widest mt-1 ${segments.length < min ? 'text-red-500' : 'text-green-500'}`}>{segments.length < min ? `Required: At least ${min} segment(s)` : 'Segment Requirement Satisfied'}</p>
                </div>
            ...
            <div className="space-y-6">
                {segments.map((seg, idx) => (
                    <div key={idx} className="p-8 bg-gray-50 rounded-[2.5rem] border-4 border-gray-100 flex items-start gap-6 group animate-in slide-in-from-right-4">
                        ...
                        <div className="flex-1 space-y-4">
                            <input className="w-full bg-white p-4 border-2 border-transparent rounded-2xl focus:border-indigo-400 outline-none font-black text-gray-800 uppercase tracking-tighter text-lg" value={seg.title} onChange={e => onUpdate(idx, {title: e.target.value})} placeholder="Segment Title (e.g. Theological Perspective)" />
                            <textarea className="w-full bg-white p-5 border-2 border-transparent rounded-2xl focus:border-indigo-400 outline-none font-medium text-gray-700 leading-relaxed min-h-[150px]" value={seg.body} onChange={e => onUpdate(idx, {body: e.target.value})} placeholder="Detailed Body Content (HTML/Markdown Supported)..." />
                        </div>
                ...
                {segments.length === 0 && <div className="text-center py-20 border-4 border-dashed border-gray-100 rounded-[2.5rem]"><Info size={48} className="mx-auto mb-4 text-gray-200" /><p className="font-black text-gray-400 uppercase tracking-widest text-sm">No structured segments defined yet.</p></div>}
            </div>
        </div>
    );
};
