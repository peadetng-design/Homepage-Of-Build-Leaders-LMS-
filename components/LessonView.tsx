...
const LessonView: React.FC<LessonViewProps> = ({ lesson, currentUser, onBack }) => {
        ...
        <div className="p-4 md:p-6 relative z-10"> 
          ...
          <div className="grid grid-cols-1 gap-2 px-2 pt-2">
            {quiz.options.map((opt) => {
              ...
              return (
                <button key={opt.id} disabled={isAnswered} onClick={() => handleOptionSelect(quiz, opt)} className={`relative p-5 md:p-8 rounded-[2rem] border-4 transition-all duration-700 text-left group/opt overflow-visible w-full ${containerClass}`}>
  ...
  return (
    <div className="max-w-6xl mx-auto pb-48 animate-in fade-in duration-700 relative min-h-screen flex flex-col">
      ...
      {/* REFINED BREADCRUMB NAVIGATION - 30% REDUCTION */}
      <nav className="flex items-center flex-wrap gap-3 mb-10 px-6 py-4 bg-white border-4 border-gray-100 shadow-xl rounded-[2rem] animate-in slide-in-from-top-4 w-fit">
          <button onClick={onBack} className="flex items-center gap-2 text-[9px] md:text-[10px] font-black text-gray-400 hover:text-royal-900 transition-all uppercase tracking-[0.2em] group">
          ...
          <div className="flex items-center gap-2">
             ...
             <span className="text-[9px] md:text-[12px] font-serif font-black text-royal-700 uppercase tracking-tight truncate max-w-[150px] md:max-w-sm">
          ...
          <div className="flex items-center gap-2">
             ...
             <span className="text-[9px] md:text-[12px] font-serif font-black text-royal-900 uppercase tracking-tight truncate max-w-[150px] md:max-w-sm">
                {lesson.title}
             </span>
          </div>
      </nav>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 flex-1">
        <div className="lg:col-span-2 space-y-16">
          ...
          {/* COMPONENT 2: LEADERSHIP NOTES */}
          {lesson.leadershipNotes && lesson.leadershipNotes.length > 0 && (
            <section className="space-y-8">
                <div className="flex items-center gap-4 mb-4">
                    <PenTool className="text-royal-800" size={32} />
                    <h2 className="text-3xl font-serif font-black text-gray-900 uppercase tracking-tighter">LEADERSHIP PERSPECTIVE</h2>
                </div>
          ...
          {/* COMPONENT 3: NOTE QUIZZES (RE-DISPLAY FIXED) */}
            ...
            {isInsightOpen && (
                <div className="w-[300px] md:w-[420px] max-h-[85vh] bg-white/95 backdrop-blur-2xl border-4 border-indigo-600 rounded-[2.5rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] flex flex-col pointer-events-auto animate-in slide-in-from-bottom-12 duration-500 overflow-hidden">
                    <div className="bg-indigo-600 p-5 text-white flex justify-between items-center shrink-0 border-b-4 border-indigo-700 shadow-md relative z-10">
                        ...
                        <button onClick={() => setIsInsightOpen(false)} className="p-2 bg-white/20 hover:bg-white/40 rounded-xl transition-all shadow-inner hover:scale-110 active:scale-95"><X size={20} strokeWidth={3}/></button>
                    </div>
                    <div className="flex-1 p-5 flex flex-col gap-4 overflow-hidden bg-white">
                        <div className="relative flex-1 flex flex-col">
                             <button onClick={() => setNoteText("")} className="absolute top-3 right-3 p-2 bg-red-100 text-red-600 rounded-xl opacity-60 hover:opacity-100 transition-opacity z-20 shadow-sm font-black text-[10px] uppercase tracking-widest" title="Purge Reflection">Clear</button>
                            <textarea value={noteText} onChange={(e) => setNoteText(e.target.value)} placeholder="Capture spiritual revelations and reflections here..." className="flex-1 w-full bg-indigo-50/20 border-4 border-indigo-50 rounded-[2rem] p-6 pt-10 outline-none font-bold text-gray-800 text-lg resize-none custom-scrollbar placeholder:text-indigo-200 focus:border-indigo-400 focus:bg-white transition-all shadow-inner"/>
                        </div>
                        <button onClick={handleSaveNote} disabled={isSavingNote} className="w-full py-6 bg-royal-900 text-white font-black rounded-3xl hover:bg-black transition-all flex items-center justify-center gap-4 uppercase text-sm tracking-[0.3em] shadow-[0_15px_30px_rgba(30,27,75,0.4)] border-b-8 border-black active:scale-95 transform hover:-translate-y-1 shrink-0">
                            {isSavingNote ? <><Loader2 className="animate-spin" size={24} /><span>SYNCHRONIZING...</span></> : <><CloudUpload size={24} className="text-gold-400" /><span>SAVE & UPLOAD TO REGISTRY</span></>} 
                        </button>
                    </div>
                </div>
            )}
            <button onClick={() => setIsInsightOpen(!isInsightOpen)} className="w-16 h-16 md:w-20 md:h-20 bg-indigo-600 text-white rounded-full shadow-2xl flex items-center justify-center hover:scale-110 active:scale-95 transition-all pointer-events-auto group border-4 border-white relative">
      ...
      {/* SEQUENTIAL NAVIGATION CONTROLS - GLOBAL CONSOLE FOOTER */}
      <div ref={bottomRef} className="mt-20 pt-20 border-t-8 border-gray-100 flex flex-col items-center gap-12 pb-32 animate-in slide-in-from-bottom-8">
          <div className="flex flex-col sm:flex-row items-stretch gap-8 w-full max-w-6xl">
              {/* PREVIOUS BUTTON - ALWAYS ENABLED */}
              <div className="flex-1">
                {adjacentLessons.prev ? (
                    <button 
                        onClick={() => navigateAdjacent(adjacentLessons.prev!)}
                        className="w-full h-full flex items-center justify-between p-10 bg-white border-4 border-royal-100 rounded-[2.5rem] hover:border-gold-500 hover:shadow-[0_25px_60px_-15px_rgba(245,158,11,0.3)] transition-all transform hover:-translate-y-2 group active:scale-95 border-b-[12px] border-royal-200"
                    >
                        <div className="p-4 bg-royal-50 text-royal-400 group-hover:bg-gold-500 group-hover:text-white rounded-[1.5rem] transition-all">
                            <ChevronLeft size={40} strokeWidth={3} />
                        </div>
                        <div className="text-right flex-1 pr-6">
                            <p className="text-[12px] font-black text-gray-300 uppercase tracking-[0.3em] mb-2 group-hover:text-gold-600 transition-colors">Registry History</p>
                            <h4 className="font-serif font-black text-royal-900 uppercase text-xl md:text-2xl leading-tight">Return Previous</h4>
                        </div>
                    </button>
                ) : (
                    <div className="w-full h-full p-10 border-4 border-dashed border-gray-100 rounded-[2.5rem] flex flex-col items-center justify-center opacity-30">
                        <Layers size={48} className="text-gray-200 mb-4" />
                        <span className="text-xs font-black text-gray-400 uppercase tracking-[0.4em]">Origin Reached</span>
                    </div>
                )}
              </div>

              {/* NEXT BUTTON - STRICTLY GUARDED POP-IN */}
              <div className="flex-1">
                {adjacentLessons.next ? (
                    <div className="relative h-full">
                        {!isLessonCompleted ? (
                            <div className="h-full w-full p-10 bg-gray-50 border-4 border-gray-200 rounded-[2.5rem] flex flex-col items-center justify-center text-center gap-4 opacity-70 border-b-[12px]">
                            ...
                            <button 
                                ...
                                className="w-full h-full flex items-center justify-between p-10 bg-royal-900 text-white border-4 border-black rounded-[2.5rem] shadow-[0_30px_70px_-15px_rgba(30,27,75,0.6)] hover:bg-black hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.8)] transition-all transform hover:-translate-y-2 group active:scale-95 border-b-[12px] border-black animate-pop-in overflow-hidden relative"
                            >
                                <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                                    <Sparkles size={80} />
                                </div>
                                <div className="text-left flex-1 pl-6 z-10">
                                    <p className="text-[12px] font-black text-gold-400 uppercase tracking-[0.3em] mb-2 group-hover:animate-pulse">Verified Pathway</p>
                                    <h4 className="font-serif font-black uppercase text-xl md:text-2xl leading-tight">Proceed Next</h4>
                                </div>
                                <div className="p-4 bg-white/10 text-white group-hover:bg-gold-500 rounded-[1.5rem] transition-all z-10 ring-4 ring-white/5">
                    ...
                    <div className="w-full h-full p-10 bg-gold-50 border-4 border-gold-200 rounded-[2.5rem] flex flex-col items-center justify-center text-center gap-4 border-b-[12px] border-gold-300">
                        <BadgeCheck size={48} className="text-gold-500" />
                        <p className="text-xs font-black text-gold-700 uppercase tracking-[0.4em]">Matrix Mastered</p>
                    </div>
                )}
              </div>
          </div>
